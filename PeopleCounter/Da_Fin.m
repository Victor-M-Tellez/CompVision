...
%Zebra8
%
C=zeros(x,y,3);% dummy clone to hold transformations we make to the original image
f= zeros(1,2000);% holder array to 

[Final,f] = AssingLabelsRound1(Final,f);
Final=AssingLabelsRound2(Final,f);
[numbersFound, NumberOfObjects] = ObjectCount(Final);

%Now we create an array to store the labels of the objects
labels=FindLabelsUsedInPic(NumberOfObjects,numbersFound);

fprintf('%i Objects found \n', NumberOfObjects);


%C=ColorLabeledObjects(x,y,Final,C,labels);
%imwrite(C,'Zebra8_end.jpeg','jpeg');
